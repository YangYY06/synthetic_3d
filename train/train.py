...
def parse_args():
    parser = argparse.ArgumentParser(description='Train Neural Mesh Model on P3D-Diffusion')
    ...
    parser.add_argument('--log_itr', type=int, default=1000)
    parser.add_argument('--save_itr', type=int, default=10000)
    ...
    args.train_img_path = os.path.join(args.dataset_path, 'train', 'images_diffusion')
    args.train_anno_path = os.path.join(args.dataset_path, 'train', 'annotations')
    args.obj_mask_path = os.path.join(args.dataset_path, 'train', 'masks')
...
def prepare_data(args):
    ...
    train_dataset = SuperCLEVRTrain(
        ...
        prefix=f'{args.filename_prefix}_train',
    ...
    #     train_dataset.file_list = train_dataset.file_list[:args.num_samples]
...
def main():
    ...
    train_dataset, train_iterator, val_dataset, val_iterator = prepare_data(args)
