...
from src.datasets import PASCAL3DPMulti, PASCAL3DPTest, PASCAL3DPOcc, OODCVTrain
...
def parse_args():
    ...
    parser.add_argument('--img_path', type=str,
                        default='/home/jiahao/pretrain_6d_pose/OOD_train')
    ...
    args.img_path = os.path.join(args.img_path, 'images', args.category)
    args.anno_path = os.path.join(args.anno_path, 'annotations', args.category)
...
def main():
    ...
    idx = np.argsort(scores)[-100:]
    img_list = []
    for i in idx:
        img_list.append(dataset[i]['img_name'])
    np.savetxt(f'{args.category}_pseudo_.txt', img_list, delimiter='\n',
               fmt='%s')
