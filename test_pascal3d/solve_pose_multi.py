...
def get_corr(px_samples, py_samples, kpt_score_map, kp_coords, kp_vis, down_sample_rate, hm_h, hm_w):
    ...
    kp_coords_curr = np.repeat(np.expand_dims(kp_coords, axis=1), len(principal_samples),
    ...
    kp_vis_curr = np.repeat(np.expand_dims(kp_vis, axis=1), len(principal_samples),
    ...
    kp_coords_curr = (kp_coords_curr[:, :, 1:2] * hm_w + kp_coords_curr[:, :, 0:1]).astype(
        np.int32)  # (num_samples, 1024, 1)
...
def get_corr_pytorch(px_samples, py_samples, kpt_score_map, kp_coords, kp_vis, down_sample_rate, hm_h, hm_w, device):
    if len(px_samples) == 50:
        l, n = 1, 50
    elif len(px_samples) == 1:
        l, n = 1, 1
    ...
    for i in range(n):
        if i != n - 1:
            px_s, py_s = torch.from_numpy(px_samples[i * l:i * l + l]).to(device), torch.from_numpy(py_samples).to(
                device)
        else:
...
def solve_pose_multi_obj(feature_map, inter_module, kp_features, clutter_bank, poses,
                         ...
                         verbose=False, down_sample_rate=8, hierarchical=0, xv=None, yv=None):
    ...
    if xv is None or yv is None:
        xv, yv = np.meshgrid(px_samples, py_samples, indexing='ij')
        corr = get_corr_pytorch(px_samples, py_samples, kpt_score_map, kp_coords, kp_vis, down_sample_rate, hm_h, hm_w,
                                device)
    ...
    corr2d_max = np.max(corr2d, axis=0)
    extrema = []
    c = corr2d[:, 0, 0].reshape(poses.shape[0], poses.shape[1], poses.shape[2], poses.shape[3])
    e_azim, e_elev, e_the, e_dist = np.unravel_index(np.argmax(c, axis=None), c.shape)
    p = poses[e_azim, e_elev, e_the, e_dist]
    extrema.append({
        'azimuth': p[0],
        ...
        'px': px_samples[0],
        'py': py_samples[0],
        'principal': [px_samples[0], py_samples[0]]
    })
    pred = {'pre_render': extrema, 'corr2d': corr2d_max}
    ...
    for i in range(len(extrema)):
        ...
        C = camera_position_from_spherical_angles(distance_pre, elevation_pre, azimuth_pre, degrees=False,
                                                  device=device)
        C = torch.nn.Parameter(C, requires_grad=True)
        ...
        refined.append({
            ...
            'px': px_pred,
            'py': py_pred,
            'principal': [px_pred * down_sample_rate + hm_w * down_sample_rate / 2,
                          py_pred * down_sample_rate + hm_h * down_sample_rate / 2]
        })
...
def resolve_occ(seg_maps, score_maps):
    ...
    seg_maps_pad = np.concatenate(
        [np.zeros((1, seg_maps_copy.shape[1], seg_maps_copy.shape[2]), dtype=seg_maps_copy.dtype), seg_maps_copy],
        axis=0)
...
def save_segmentation_maps(segmentation_maps, path):
    b, h, w = segmentation_maps.shape
    m = np.zeros((h, w, 3), dtype=np.uint8)
    for i in range(b):
        m[segmentation_maps[i] == 1] = colors[i + 1]
    m = cv2.resize(m, (w * 8, h * 8), interpolation=cv2.INTER_NEAREST)
