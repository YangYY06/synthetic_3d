...
class OODCVTrain(Dataset):
    def __init__(self, img_path, anno_path, mesh_path, category, dist=6, image_h=352, image_w=520,
                 crop_object=False,
                 transform=None, bg_path=None, pseudo=None, file_list=None):
        super().__init__()
        ...
        if file_list is not None:
            self.file_list = [l.strip() for l in open(file_list).readlines()]
        else:
            self.file_list = [file_name.split('.')[0] for file_name in os.listdir(img_path)]
    ...
    def __getitem__(self, item):
        ...
        if img.mode != 'RGB':
            img = img.convert('RGB')
        img = np.array(img)
        anno = dict(np.load(os.path.join(self.anno_path, fname + '.npz')))

        azimuth_ori = anno['azimuth'].item()
        elevation = anno['elevation'].item()
        theta = anno['theta'].item()

        if self.pseudo is not None:
            labels = self.pseudo
            azimuth_ori = labels[fname]['azimuth']
            theta = labels[fname]['theta']
            elevation = labels[fname]['elevation']

        fine_cad_idx = anno['cad_index'].item()

        resize_rate = 5.0 / self.dist
        ...
        azimuth = (azimuth_ori - np.pi) % (2 * np.pi)
        # azimuth = np.pi * 5 / 6
        # elevation = np.pi / 9
        # theta = 0
        ...
        principal = np.array([self.image_w // 2, self.image_h // 2])
        ...
        sample = {
            ...
            'theta': theta,
            'distance': self.dist,
            'distance_orig': anno['distance'].item(),
    ...
    def debug(self, item, fname='debug.png'):
        ...
        print(sample['principal'], sample['distance_orig'])
        img = sample['img']
        # img = sample['obj_mask']
        kp = sample['kp']
        kpvis = sample['kpvis']
        p1, p2 = sample['principal']
        img = cv2.circle(img, (int(p1), int(p2)), 2, (255, 0, 0), -1)
