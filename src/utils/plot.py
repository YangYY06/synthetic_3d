...
from pytorch3d.renderer import OpenGLPerspectiveCameras, RasterizationSettings, MeshRasterizer, MeshRenderer, \
    PointLights, HardPhongShader, PerspectiveCameras
...
def plot_mesh(img, mesh_path, azimuth, elevation, theta, distance, principal, down_sample_rate=8, focal_length=3000,
              fuse=True, size=(1, 1, 1), color=[1, 0.85, 0.85]):
    h, w, c = img.shape
    ...
    cameras = PerspectiveCameras(focal_length=focal_length,
                                 ...
                                 principal_point=((render_image_size // 2,
                                                   render_image_size // 2),), in_ndc=False,
                                 device='cuda:0')
    ...
    verts = pre_process_mesh_pascal(verts).to(torch.float32)
...
def plot_multi_mesh(img, mesh_path, pred, down_sample_rate=8, focal_length=3000, fuse=True, size=1.0,
                    color=[1, 0.85, 0.85]):
    img_list = [plot_mesh(img, mesh_path, p['azimuth'], p['elevation'], p['theta'], p['distance'], p['principal'],
                          down_sample_rate, focal_length=focal_length, fuse=False, size=size, color=color) for p in
                pred]
    s = img_list[0]
...
def plot_loss_landscape(center, feature_map, inter_module, clutter_bank, out_path, use_z=False,
    ...
    for azim_s in azimuth_shifts:
        C = camera_position_from_spherical_angles(distance_center, elevation_center, azimuth_center + azim_s,
        ...
        flow = torch.tensor([-(px_center - hm_w * down_sample_rate / 2) / down_sample_rate,
                             -(py_center - hm_h * down_sample_rate / 2) / down_sample_rate], dtype=torch.float32).to(
            device)
    ...
    for elev_s in elevation_shifts:
        C = camera_position_from_spherical_angles(distance_center, elevation_center + elev_s, azimuth_center,
                                                  degrees=False, device=device)
        theta = torch.tensor(theta_center, dtype=torch.float32).to(device)
        projected_map = inter_module(C, theta, mode=mode, blur_radius=blur_radius).squeeze()
        flow = torch.tensor([-(px_center - hm_w * down_sample_rate / 2) / down_sample_rate,
    ...
    for theta_s in theta_shifts:
        C = camera_position_from_spherical_angles(distance_center, elevation_center, azimuth_center, degrees=False,
                                                  device=device)
    ...
    for dist_s in distance_shifts:
        C = camera_position_from_spherical_angles(distance_center + dist_s, elevation_center, azimuth_center,
                                                  degrees=False, device=device)
        theta = torch.tensor(theta_center, dtype=torch.float32).to(device)
        projected_map = inter_module(C, theta, mode=mode, blur_radius=blur_radius).squeeze()
        flow = torch.tensor([-(px_center - hm_w * down_sample_rate / 2) / down_sample_rate,
    ...
    for px_s in px_shifts:
        ...
        theta = torch.tensor(theta_center, dtype=torch.float32).to(device)
        projected_map = inter_module(C, theta, mode=mode, blur_radius=blur_radius).squeeze()
        flow = torch.tensor([-(px_center + px_s - hm_w * down_sample_rate / 2) / down_sample_rate,
                             -(py_center - hm_h * down_sample_rate / 2) / down_sample_rate], dtype=torch.float32).to(
            device)
        flow_map = flow.view(1, 2, 1, 1).repeat(1, 1, hm_h, hm_w)
        projected_map = flow_warp(projected_map.unsqueeze(0), flow_map)[0]
    ...
    for py_s in py_shifts:
        ...
        projected_map = inter_module(C, theta, mode=mode, blur_radius=blur_radius).squeeze()
        flow = torch.tensor([-(px_center - hm_w * down_sample_rate / 2) / down_sample_rate,
                             -(py_center + py_s - hm_h * down_sample_rate / 2) / down_sample_rate],
