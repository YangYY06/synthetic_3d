...
def notation_blender_to_pyt3d(mesh_path, sample, anno, image_h, image_w):
        ...
        for elev in elev_sample:
            for theta in theta_sample:
                for dist in dist_sample:
                    x0 = np.array([azim, elev, theta, dist, center[0] / 520, center[1] / 352])
                    res = least_squares(fun, x0, x_scale=[2 * np.pi, 2 * np.pi, 2 * np.pi, 20, 1.0, 1.0], ftol=1e-3,
                                        method='trf', args=(xvert_orig, pts_2d))
    ...
    pose = {
        ...
        'theta': min_x[2],
        'distance': min_x[3],
        'principal': np.array([min_x[4] * 520, min_x[5] * 352])
    }
