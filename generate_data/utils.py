...
def add_object3d(object_dir, name, obj_pth, scale, loc, theta=0, roll=0, pitch=0):
    """
    Load an object from a file. We assume that in the directory object_dir, there
    is a file named "$name.blend" which contains a single object named "$name"
    that has unit size and is centered at the origin.
    ...
    # Set the new object as active, then rotate, scale, and translate it
    bpy.context.scene.objects.active = bpy.data.objects[new_name]
    bpy.context.object.rotation_euler[0] = roll / 180. * math.pi
    bpy.context.object.rotation_euler[1] = pitch / 180.0 * math.pi
    bpy.context.object.rotation_euler[2] = theta / 180. * math.pi
    bpy.ops.transform.resize(value=scale)
    ...
    # min_z = min((mw * v.co).z for v in current_obj.data.vertices)
...
def modify_part_color(current_obj, part_name, part_verts_idxs, mat_list, material_name, color_name, color, texture,
                      mat_freq):
    for i, mat in enumerate(current_obj.data.materials):
