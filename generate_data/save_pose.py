...
from src.datasets import PASCAL3DPMulti, PASCAL3DPTest, SuperCLEVRTest
...
def main():
    ...
    for i in tqdm(range(len(dataset_p3d))):
        ...
        azi.append(sample['azimuth'])
        theta.append(sample['theta'])

    plt.figure(figsize=(20, 5))
    plt.hist(np.array(azi) * 180 / np.pi, bins=np.linspace(0, 360, 37, endpoint=True))
    ax = plt.gca()
    ax.set_xticks(np.linspace(0, 360, 37, endpoint=True))
    plt.savefig(f'{args.category}_azi.png')
    plt.close()

    plt.figure(figsize=(20, 5))
    plt.hist(np.array(theta) * 180 / np.pi, bins=np.linspace(-180, 180, 370, endpoint=True))
    ax = plt.gca()
    ax.set_xticks(np.linspace(-180, 180, 37, endpoint=True))
    plt.savefig(f'{args.category}_theta.png')
    plt.close()
