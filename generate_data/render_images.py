...
def render_scene(args,
    ...
    if args.save_part_mask:
        objects, blender_objects, p_colors, path = add_random_objects(scene_struct, num_objects, args, camera, idx,
                                                                      dtd_fnames)
    else:
    ...
    if dtd_fnames is not None:
        for i in range(num_objects):
            ...
            for j in range(len(current_obj.data.materials)):
                ...
                texture_img.image = bpy.data.images.load(dtd_fnames[j % 10])

                links.new(bsdf.inputs["Base Color"], texture_img.outputs["Color"])
                links.new(material_output.inputs["Surface"], bsdf.outputs["BSDF"])

                current_obj.data.materials[j] = img_mat

    if args.save_part_mask:
        return objects, blender_objects, part_colors, path
    else:
        return objects, blender_objects
...
def check_visibility(blender_objects, min_pixels_per_object, min_pixels_per_part=None, is_part=False, obj_info=None,
    ...
    print('color_count_obj:', len(color_count_obj))
    print('blender_objects:', len(blender_objects))
